...
// ------------------- DATE, TIME & ALERT FUNCTIONALITY ------------------- //

// Step 1: Add date/time input fields dynamically
...
timeInput.type = "time";
timeInput.id = "todoTime";
timeInput.style.marginLeft = "5px";
timeInput.required = false;

form.insertBefore(dateInput, form.querySelector("button"));
form.insertBefore(timeInput, form.querySelector("button"));

// Extend addTodo functionality — without touching the original
...
form.addEventListener("submit", function (e) {
  ...
  // Attach date/time info to last added todo
  if (todoText && !todoArray.some(t => t.todoItem === todoText && t.todoDate)) {
    const last = todoArray.find(t => t.todoItem === todoText);
    if (last) {
      last.todoDate = date || "";
      last.todoTime = time || "";
      saveToLocalStorage(todoArray);
...
// Step 2: Display date/time under each task
const originalDisplayTodo = displayTodo;
displayTodo = function (todoArray) {
  originalDisplayTodo(todoArray);

  const listItems = document.querySelectorAll(".todo-item");
  listItems.forEach((li, index) => {
    const todo = todoArray[index];
    if (todo && (todo.todoDate || todo.todoTime)) {
      let info = li.querySelector(".datetime-info");
      if (!info) {
        info = document.createElement("small");
        info.classList.add("datetime-info");
        info.style.color = "#888";
        info.style.display = "block";
        info.style.marginLeft = "30px";
        li.appendChild(info);
      }
      info.textContent = `${todo.todoDate || ""} ${todo.todoTime || ""}`;
    }
  });
};

// Step 3: Alert Reminder system (runs every minute)
setInterval(() => {
  const now = new Date();

  // ✅ Use local date format so date change works
  const currentDate = now.toLocaleDateString("en-CA"); // YYYY-MM-DD
  const currentTime = now.toTimeString().slice(0, 5); // HH:MM format

  todoArray.forEach((todo) => {
    if (
      todo.todoDate === currentDate &&
      todo.todoTime === currentTime &&
      !todo.alertShown
    ) {
      alert(`⏰ Reminder: "${todo.todoItem}" is scheduled for now!`);
...
// ------------------- END OF DATE, TIME & ALERT ADDITIONS ------------------- //
